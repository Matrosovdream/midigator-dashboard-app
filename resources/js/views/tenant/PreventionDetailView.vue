...
async function load() {
    loading.value = true;
    try {
        const res = await api.get(`/api/v1/preventions/${route.params.id}`);
...
async function resolve(outcome: string) {
    await api.post(`/api/v1/preventions/${route.params.id}/resolve`, { outcome });
    load();
}
    ...
    <div class="grid grid-cols-12 gap-4">
        <div class="col-span-12">
            <div class="card !mb-0 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                <div>
                    <Button icon="pi pi-arrow-left" text @click="router.push({ name: 'tenant.preventions.index' })" />
                    <span class="font-semibold text-2xl ml-2">Prevention {{ data?.alert_id ?? `#${route.params.id}` }}</span>
                </div>
                <div class="flex flex-wrap gap-2 items-center">
                    <AssignPicker
                        v-if="data"
                        target-type="prevention"
                        :target-id="route.params.id as string"
                        :model-value="data.assignee"
                        @update:model-value="(v) => (data.assignee = v)"
                    />
                    <Button label="Refund" icon="pi pi-check" severity="success" @click="resolve('refunded')" />
                    <Button label="Decline" icon="pi pi-times" severity="danger" @click="resolve('declined')" />
                </div>
            </div>
        </div>
        ...
        <div class="col-span-12 xl:col-span-8">
            <Card>
                ...
                <template #content>
                    ...
                    <div v-else class="grid grid-cols-2 gap-3">
                        <div><span class="text-muted-color text-xs">Provider</span><div>{{ data.provider ?? '—' }}</div></div>
                        ...
                        <div><span class="text-muted-color text-xs">Received</span><div>{{ data.created_at ? new Date(data.created_at).toLocaleString() : '—' }}</div></div>
                    </div>
                </template>
            </Card>
            ...
            <Card class="mt-4">
                ...
                <template #content>
                    <CaseComments target-type="prevention" :target-id="route.params.id as string" :initial="data?.comments" />
                </template>
            </Card>
        </div>
